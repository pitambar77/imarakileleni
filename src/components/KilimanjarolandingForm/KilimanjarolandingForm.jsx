...
const KilimanjarolandingForm = ({ editData, onSuccess }) => {
  ...
  const [adventure, setAdventure] = useState([
    { heading: "", section: [{ title: "", subtitle: "", description: "", image: null, imagePreview: null }] },
  ]);
  ...
  // *------------------------------------------------------*
  const addOverview = () =>
    setOverviewinfo([...overviewinfo, { title: "", description: [], image: null }]);
  ...
  const addAdventureSection = (i) => {
    const updated = [...adventure];
    updated[i].section.push({ title: "", subtitle: "", description: "", image: null });
    setAdventure(updated);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    adventure.forEach((x) => x.section.forEach((y) => y.image && data.append("adventureImages", y.image)));
  ...
  return (
    <form className="grid grid-cols-2 gap-4 pb-20" onSubmit={handleSubmit}>
      ...
      <div className="col-span-2">
        ...
        {mainImagePreview && <img src={mainImagePreview} className="w-48 rounded mt-2" />}
      </div>
      ...
      <section className="col-span-2 mt-5">
        <div className="flex justify-between">
          ...
          <button type="button" className="bg-green-600 text-white px-3 py-1 rounded"
            onClick={addOverview}>+ Add</button>
        </div>
        ...
        {overviewinfo.map((item, i) => (
          <div key={i} className="border bg-gray-50 p-4 rounded mt-3">
            <input className="border p-2 w-full mb-2" name="title" placeholder="Title"
              value={item.title} onChange={(e)=>handleOverviewBase(i,e)} />

            <button type="button" className="bg-blue-600 text-white px-3 py-1 rounded"
              onClick={()=>addOverviewBlock(i)}>+ Add Description Block</button>
            ...
            {item.description.map((block,j)=>(
              <div key={j} className="border bg-white p-2 rounded mt-2">
                ...
                <button type="button" className="bg-red-600 text-white px-3 py-1 rounded mt-2"
                  onClick={()=>removeOverviewBlock(i,j)}>Remove</button>
              </div>
            ))}

            <input type="file" className="border p-2 mt-2"
      ...
      <section className="col-span-2 mt-6">
        <div className="flex justify-between">
          ...
          <button type="button" className="bg-green-600 text-white px-3 py-1 rounded"
            onClick={addAdventure}>+ Add</button>
        </div>

        {adventure.map((item, i)=>(
          <div key={i} className="border p-4 bg-gray-50 rounded mt-3">
            <input className="border p-2 w-full mb-3" placeholder="Heading"
              name="heading" value={item.heading}
              onChange={(e)=>handleAdventure(i,e)} />

            <button type="button" className="bg-blue-600 text-white px-3 py-1 rounded"
              onClick={()=>addAdventureSection(i)}>+ Add Section</button>

            {item.section.map((sec,j)=>(
              <div key={j} className="border p-3 bg-white rounded mt-2">
                <input className="border p-2 w-full mb-2" name="title" placeholder="Title"
                  value={sec.title} onChange={(e)=>handleAdventureSection(i,j,e)} />

                <input className="border p-2 w-full mb-2" name="subtitle" placeholder="Subtitle"
                  value={sec.subtitle} onChange={(e)=>handleAdventureSection(i,j,e)} />

                <textarea className="border p-2 w-full mb-2" name="description"
                  placeholder="Description"
                  value={sec.description} onChange={(e)=>handleAdventureSection(i,j,e)} />

                <input type="file" className="border p-2"
                  onChange={(e)=>handleAdventureImage(i,j,e)} />

                {sec.imagePreview && <img src={sec.imagePreview} className="w-32 mt-2" />}

                <button type="button" className="bg-red-600 text-white px-3 py-1 rounded mt-2"
                  onClick={()=>removeAdventureSection(i,j)}>Remove</button>
              </div>
            ))}

            <button type="button" className="bg-red-600 text-white px-3 py-1 rounded mt-3"
              onClick={()=>removeAdventure(i)}>Remove Section</button>
          </div>
        ))}
      </section>
      ...
      <section className="col-span-2 mt-6">
        <div className="flex justify-between">
          ...
          <button type="button" className="bg-green-600 px-3 py-1 text-white rounded"
            onClick={addWhenvisit}>+ Add</button>
        </div>

        {whenvisit.map((item, i)=>(
          <div key={i} className="border p-4 bg-gray-50 rounded mt-3">
            <input className="border p-2 w-full mb-2" placeholder="Heading"
              name="heading" value={item.heading}
              onChange={(e)=>handleWhenvisit(i,e)} />

            <button type="button" className="bg-blue-600 text-white px-3 py-1 rounded"
              onClick={()=>addMonth(i)}>+ Add Month</button>

            {item.months.map((m,j)=>(
              <div key={j} className="border mt-3 bg-white p-3 rounded">
                <input className="border p-2 w-full mb-2" name="monthname" placeholder="Month"
                  value={m.monthname} onChange={(e)=>handleMonth(i,j,e)} />

                <input className="border p-2 w-full mb-2" name="title" placeholder="Title"
                  value={m.title} onChange={(e)=>handleMonth(i,j,e)} />

                <button type="button" className="bg-blue-600 text-white px-3 py-1 rounded"
                  onClick={()=>addMonthDescription(i,j)}>+ Add Description</button>
                ...
                {m.description.map((d,k)=>(
                  <div key={k} className="border p-2 rounded mt-2">
                    ...
                    <button type="button" className="bg-red-600 text-white px-3 py-1 rounded mt-2"
                      onClick={()=>removeMonthDesc(i,j,k)}>Remove</button>
                  </div>
                ))}

                <button type="button" className="bg-red-600 text-white px-3 py-1 rounded mt-2"
                  onClick={()=>removeMonth(i,j)}>Remove Month</button>
              </div>
            ))}

            <button type="button" className="bg-red-600 px-3 py-1 rounded mt-3 text-white"
              onClick={()=>removeWhenvisit(i)}>Remove Section</button>
          </div>
        ))}
      </section>
        ...
        {faq.map((item,i)=>(
          <div key={i} className="border p-4 rounded bg-gray-50 mt-4">
            ...
            {item.answer.map((ans,j)=>(
              <div key={j} className="border bg-white p-2 rounded mt-2">
                <select className="border p-2 w-full mb-2" name="type"
                  value={ans.type} onChange={(e)=>handleFaqAnswer(i,j,e)}>
                  <option value="header">Header</option>
                  <option value="paragraph">Paragraph</option>
                  <option value="list">List</option>
                </select>
            ...
            <button type="button" className="bg-red-600 text-white px-3 py-1 rounded mt-4"
              onClick={()=>removeFaq(i)}>Remove Question</button>
          </div>
        ))}
      </section>

      <button type="submit" className="bg-blue-600 col-span-2 text-white py-2 rounded mt-5">
