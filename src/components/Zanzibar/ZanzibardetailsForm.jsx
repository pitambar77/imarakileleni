...
const ZanzibardetailsForm = ({ editData, onSuccess }) => {
...
/* ---------- OTHERS ---------- */
  ...
  adventure.forEach(
    ...
    try {
      let res;
      if (editData) {
        res = await API.put(`/zanzibardetails/${editData._id}`, data);
      } else {
        res = await API.post("/zanzibardetails", data);
      }
      alert("Saved Successfully!");
      onSuccess && onSuccess(res.data);
    } catch (err) {
      console.error(err);
      alert("Error saving zanzibar details");
    }
  };
...
export default ZanzibardetailsForm