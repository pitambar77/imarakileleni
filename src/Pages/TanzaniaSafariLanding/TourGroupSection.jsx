...
const navigate = useNavigate();
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        const res = await API.get("/travelgroup"); // 🔁 confirm endpoint
    ...
    <section className="bg-[#fedec8] py-8 md:py-16 px-4 md:px-10 lg:px-16 xl:px-18 2xl:px-28">
      ...
      <div className="bg-[#fedec7]">
        <div className="max-w-7xl mx-auto px-6 md:px-10 mt-12 md:mt-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 items-center">
            <Feature icon={<FaWallet />} text="Save your seat instantly with a small deposit" />
            <Feature icon={<FaHourglassHalf />} text="Choose payment timings that match your schedule" />
            <Feature icon={<FaCalendarAlt />} text="Make itinerary changes without stress before departure" />
            <Feature icon={<FaShieldAlt />} text="Your booking funds are always fully safeguarded" />
          </div>
